...
Мест, где версия записана, два: `plugin.json` пакета и его запись в
`marketplace.json`. Раньше их было пятнадцать — по два на каждый из семи
пакетов плюс связка, — и именно эта арифметика была одной из причин свести
всё в один пакет: правка руками третий релиз подряд однажды делается наспех,
а пропущенное место означает пакет, который не обновится.

Скрипт остался и после слияния, и не по инерции: расхождение между версией
пакета и версией записи в маркетплейсе всё ещё возможно, а последствие у него
прежнее — движок сравнивает версии и решает, что обновлять нечего. Двух мест
достаточно, чтобы разойтись.

Расхождение версий ДО подъёма — отказ, а не повод «выровнять заодно»: дерево,
версии которого разошлись, собрано не тем способом, каким собирается обычно, и
