...
class ЧужаяПлатформа(unittest.TestCase):
    ...
    def test_названо_что_именно_не_работает(self):
        ...
        self.assertTrue(что, "список поломок пуст — сказать человеку нечего")
        # Тормоз и все четыре хука переехали на Python и работают без
        # оболочки. Держать их в списке поломок значит врать в другую сторону:
        # человек решит, что не работает то, что работает.
        for ушедшее in ("тормоз", "хуки"):
            with self.subTest(ушедшее):
                self.assertNotIn(ушедшее, что,
                                 f"{ушедшее} больше не зависит от оболочки")
        for b in v["broken"]:
