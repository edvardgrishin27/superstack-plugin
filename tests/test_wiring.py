        ...
        for skill in sorted((PKG / "skills").glob("*")):
            ...
            for f in sorted(skill.glob("phases/*.md")):
                with self.subTest(skill=skill.name, phase=f.name):
                    ...
                    # Инвариант — не конкретное слово, а возможность решить
                    # СУДЬБУ ФАЙЛА, не открывая его. Ответов ровно три, и
                    # каждый честен по-своему: фаза условная, фаза читается
                    # в свой момент, фаза читается всегда (и тогда её вес
                    # считается стартовым — вынести текст в файл и читать
                    # его каждый раз облегчением не является).
                    маркеры = ("Нужна, если", "Читается на входе в фазу",
                               "читается всегда")
                    self.assertTrue(
                        any(м in рядом for м in маркеры),
                        f"{skill.name}/{f.name}: в SKILL.md не сказано, "
                        f"читать ли этот файл. Нужен один из: {маркеры}")
