...
#: Ненулевой код возврата не всегда провал. Гейт, доказавший свою работу отказом
#: («сборка выпуска не пустила пример наполнения»), — это УСПЕХ проверки, и
#: числом он неотличим от поломки.
#:
#: Угадывать здесь нельзя: «exit 1» с оправданием в скобках и «exit 1» от
#: сломанного набора выглядят одинаково. Поэтому исход называется дословно —
#: пометкой, которую исполнитель ставит осознанно и за которую отвечает.
#: Она снимает только тот прогон, рядом с которым стоит.
_EXPECTED = re.compile(r"ожидаемый отказ", re.I)
...
def check(text: str, task: str = None, handoffs: int = 0) -> dict:
    ...
    # У выражения две ветки — «2 failed» и «exit 1», — и совпасть может любая.
    # Брать всегда первую группу значило падать с трассировкой на второй:
    # именно так контракт рухнул на честном возврате, где было написано
    # «npm run build:release → exit 1 (пример наполнения)». Инструмент,
    # падающий вместо вердикта, хуже строгого: он не судит вовсе.
    red_n = 0
    if red:
        число = next((g for g in red.groups() if g and g.isdigit()), "0")
        red_n = int(число)
        # Пометка снимает ТОЛЬКО тот прогон, рядом с которым стоит: ищем её в
        # окне после совпадения, а не по всей строке — иначе одна пометка
        # оправдала бы все провалы блока разом.
        хвост = tests[red.end():red.end() + 60]
        if _EXPECTED.search(хвост):
            red_n = 0
            # После снятия ищем настоящий провал дальше по строке.
            for m in _RED.finditer(tests[red.end():]):
                ещё = next((g for g in m.groups() if g and g.isdigit()), "0")
                если_помечен = _EXPECTED.search(
                    tests[red.end() + m.end():red.end() + m.end() + 60])
                if int(ещё) > 0 and not если_помечен:
                    red_n = int(ещё)
                    break
